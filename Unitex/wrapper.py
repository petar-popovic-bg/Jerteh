...
class Unitex:
    ...
    def get_stats(self, file):
        ...
        if self.lang == 'Serbian-Latin':
            ...
            os.system('%s/App/UnitexToolLogger Dico -t%s -a%s/%s/Alphabet.txt %s/%s/Dela/dela-en-public.bin %s/%s/Dela/'
                      'Dnum.fst2 -qutf8-no-bom' % (self.UNITEX_DIR, snt_f, self.UNITEX_DIR, self.lang,
        ...
        os.system('%s/App/UnitexToolLogger SortTxt %s/err -l%s/err.n -o%s/%s/Alphabet_sort.txt -qutf8-no-bom' %
                  (self.UNITEX_DIR, snt_dir, snt_dir, self.UNITEX_DIR, self.lang))
        os.system('%s/App/UnitexToolLogger SortTxt %s/tags_err -l%s/tags_err.n -o%s/%s/Alphabet_sort.txt -qutf8-no-bom'
                  % (self.UNITEX_DIR, snt_dir, snt_dir, self.UNITEX_DIR, self.lang))
    ...
    def add_sentence_xml(self, file):
        ...
        os.system('%s/App/UnitexToolLogger Fst2Txt -t%s %s/%s/Graphs/Preprocessing/Sentence/Sentence-xml-ELTeC.fst2 '
                  '-a%s/%s/Alphabet.txt -M --input_offsets=%s/normalize.out.offsets --output_offsets=%s/normalize.ou'
                  't.offsets -qutf8-no-bom' % (self.UNITEX_DIR, file, self.UNITEX_DIR, self.lang, self.UNITEX_DIR,
        ...
        os.system('%s/App/UnitexToolLogger Grf2Fst2 %s/%s/Graphs/Preprocessing/Replace/Replace.grf -y --alphabet=%s/%s '
                  'Alphabet.txt -qutf8-no-bom' % (self.UNITEX_DIR, self.UNITEX_DIR, self.lang, self.UNITEX_DIR,
                                                  self.lang))
        # "/home/petar/Unitex-GramLab-3.2/App/UnitexToolLogger" Fst2Txt "-t/home/petar/Desktop/uplaodme.snt" "/home/pet
        ...
        # bom
        os.system('%s/App/UnitexToolLogger Fst2Grf2 -t%s %s/%s/Graphs/Preprocessing/Replace/Replace.fst2 -a/%s/%s/Aplha'
                  'bet.txt -R --input_offsets=%s/normalize.out.offsets --output_offsets=%s/normalize.out.offsets -qutf8'
                  '-no-bom' % (self.UNITEX_DIR, file, self.UNITEX_DIR, self.lang, self.UNITEX_DIR, self.lang,
                               snt_dir, snt_dir))
        ...
        # o-bom
        os.system('%s/App/UnitexToolLogger Tokenize %s -a%s/%s/Alphabet.txt --input_offsets=%s/normalize.out.offsets '
                  '--output_offsets=%s/tokenize.out.offsets -qutf8-no-bom' % (self.UNITEX_DIR, snt_f,
                                                                              self.UNITEX_DIR, self.lang, snt_dir,
                                                                              snt_dir))
